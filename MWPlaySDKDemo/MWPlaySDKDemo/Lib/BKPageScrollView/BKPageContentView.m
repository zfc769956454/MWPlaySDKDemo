...
@interface BKPageContentView ()<UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource> {
    CGFloat   _oldOffSetX;
...
// 用于处理重用和内容的显示
@property (strong, nonatomic) BKPageCollectionView *collectionView;
// collectionView的布局
...
@implementation BKPageContentView
...
#if DEBUG
            NSLog(@"\n请注意: 如果您希望所有的子控制器的view的系统生命周期方法被正确的调用\n请重写%@的'shouldAutomaticallyForwardAppearanceMethods'方法 并且返回NO\n当然如果您不做这个操作, 子控制器的生命周期方法将不会被正确的调用\n如果您仍然想利用子控制器的生命周期方法, 请使用'ZJScrollPageViewChildVcDelegate'提供的代理方法\n或者'BKPageScrollPageViewDelegate'提供的代理方法", [parentViewController class]);
#endif
        }
        [self commonInit];
...
- (void)commonInit {
    ...
    if ([navi isKindOfClass:[UINavigationController class]]) {
        ...
        if (navi.interactivePopGestureRecognizer) {
            ...
            [self.collectionView setupScrollViewShouldBeginPanGestureHandler:^BOOL(BKPageCollectionView *collectionView, UIPanGestureRecognizer *panGesture) {
    ...
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<ZJScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        ...
        if (strongSelf) {
            if (childVc != strongSelf.currentChildVc) {
                ...
                [BKPageContentView removeChildVc:childVc];
            }
        }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if(scrollView.contentOffset.x < 100 &&
       self.segmentView.segmentStyle.scrollLineWidth > 0){
         [self.segmentView reformScrollLineViewPositionCurrentIndex:currentIndex];
    }
}
...
- (BKPageCollectionView *)collectionView {
    if (_collectionView == nil) {
        BKPageCollectionView *collectionView = [[BKPageCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:self.collectionViewLayout];
