...
@interface BKPageScrollSegmentView ()<UIScrollViewDelegate> {
...
@implementation BKPageScrollSegmentView
...
{
#if DEBUG
    NSLog(@"BKPageScrollSegmentView ---- 销毁");
...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    BKPageTitleView *currentLabel = (BKPageTitleView *)tapGes.view;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        BKPageTitleView *titleView = [[BKPageTitleView alloc] initWithFrame:CGRectZero];
        titleView.tag = index;
        ...
        if(index == 1){
            titleView.badgeViewShow = self.segmentStyle.badgeViewShow;
        }
...
- (void)setupUI {
    ...
    {
        UIView *lineV=[[UIView alloc]initWithFrame:CGRectMake(0, self.mj_h-self.segmentStyle.bottomLineHeight, kScreenWidth, self.segmentStyle.bottomLineHeight)];
        lineV.backgroundColor=self.segmentStyle.bottomLineColor;
    ...
    if (self.segmentStyle.isScrollTitle) { // 设置滚动区域
        BKPageTitleView *lastTitleView = (BKPageTitleView *)self.titleViews.lastObject;
...
- (void)setupScrollViewAndExtraBtn {
    ...
    //        scrollW = maxX;
    //    }
    self.scrollView.frame = CGRectMake(0.0, 0.0, scrollW, self.mj_h);

    if (self.extraBtn) {
        self.extraBtn.frame = CGRectMake(scrollW , extraBtnY, extraBtnW, self.mj_h - 2*extraBtnY);
    }
}
...
- (void)setUpTitleViewsPosition {
    ...
    CGFloat titleY = 0.0;
    CGFloat titleW = 0.0;
    CGFloat titleH = self.mj_h - self.segmentStyle.scrollLineHeight;
    
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        titleW = self.scrollView.bounds.size.width / self.titles.count;
        
        NSInteger index = 0;
        for (BKPageTitleView *titleView in self.titleViews) {
    ...
    BKPageTitleView *currentTitleView = (BKPageTitleView *)self.titleViews[_currentIndex];
...
- (void)setupScrollLineAndCover {
    ...
    BKPageTitleView *firstLabel = (BKPageTitleView *)self.titleViews[0];
    CGFloat coverX = firstLabel.mj_x;
    CGFloat coverW = firstLabel.mj_w;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX , self.mj_h - self.segmentStyle.scrollLineHeight, coverW , self.segmentStyle.scrollLineHeight);
            ...
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                if(self.segmentStyle.scrollLineWidth>0){
                    ...
                    coverX = (firstLabel.mj_w - coverW) * 0.5;
                }else{
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    BKPageTitleView *oldTitleView = (BKPageTitleView *)self.titleViews[_oldIndex];
    BKPageTitleView *currentTitleView = (BKPageTitleView *)self.titleViews[_currentIndex];
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                weakSelf.scrollLine.mj_x = currentTitleView.mj_x;
                weakSelf.scrollLine.mj_w = currentTitleView.mj_w;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    CGFloat scrollLineW = 0;
                    if(self.segmentStyle.scrollLineWidth>0){
                        scrollLineW = self.segmentStyle.scrollLineWidth;
                    }else{
                        scrollLineW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                    }
                    CGFloat scrollLineX = currentTitleView.mj_x + (currentTitleView.mj_w - scrollLineW) * 0.5;
                    weakSelf.scrollLine.mj_x = scrollLineX;
                    weakSelf.scrollLine.mj_w = scrollLineW;
                } else {
                    weakSelf.scrollLine.mj_x = currentTitleView.mj_x;
                    weakSelf.scrollLine.mj_w = currentTitleView.mj_w;
                }
        ...
        if (weakSelf.coverLayer) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                weakSelf.coverLayer.mj_x = currentTitleView.mj_x - xGap;
                weakSelf.coverLayer.mj_w = currentTitleView.mj_w + wGap;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    CGFloat coverW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                    CGFloat coverX = currentTitleView.mj_x + (currentTitleView.mj_w - coverW) * 0.5;
                    weakSelf.coverLayer.mj_x = coverX;
                    weakSelf.coverLayer.mj_w = coverW;
                } else {
                    weakSelf.coverLayer.mj_x = currentTitleView.mj_x;
                    weakSelf.coverLayer.mj_w = currentTitleView.mj_w;
                }
            }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    BKPageTitleView *oldTitleView = (BKPageTitleView *)self.titleViews[oldIndex];
    BKPageTitleView *currentTitleView = (BKPageTitleView *)self.titleViews[currentIndex];

    
    CGFloat xDistance = currentTitleView.mj_x - oldTitleView.mj_x;
    CGFloat wDistance = currentTitleView.mj_w - oldTitleView.mj_w;
    
    if (self.scrollLine) {
        
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.mj_x = oldTitleView.mj_x + xDistance * progress;
            self.scrollLine.mj_w = oldTitleView.mj_w + wDistance * progress;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                CGFloat oldScrollLineW = 0;
                if(self.segmentStyle.scrollLineWidth > 0){
                    oldScrollLineW = self.segmentStyle.scrollLineWidth;
                }else{
                    oldScrollLineW = [self.titleWidths[oldIndex] floatValue] + wGap;
                }
                CGFloat currentScrollLineW = [self.titleWidths[currentIndex] floatValue] + wGap;
                wDistance = currentScrollLineW - oldScrollLineW;
                
                CGFloat oldScrollLineX = oldTitleView.mj_x + (oldTitleView.mj_w - oldScrollLineW) * 0.5;
                CGFloat currentScrollLineX = currentTitleView.mj_x + (currentTitleView.mj_w - currentScrollLineW) * 0.5;
                xDistance = currentScrollLineX - oldScrollLineX;
                if(self.segmentStyle.scrollLineWidth > 0)
                {
                    self.scrollLine.mj_x = oldScrollLineX + xDistance * progress+wDistance/2;
                    self.scrollLine.mj_w = oldScrollLineW ;
                    if(self.scrollLine.centerX > MAX(oldTitleView.centerX, currentTitleView.centerX)){
                         self.scrollLine.centerX = MAX(oldTitleView.centerX, currentTitleView.centerX);
                    }
                    
                }else{
                    self.scrollLine.mj_x = oldScrollLineX + xDistance * progress;
                    self.scrollLine.mj_w = oldScrollLineW + wDistance * progress;
                }
            } else {
                self.scrollLine.mj_x = oldTitleView.mj_x + xDistance * progress;
                self.scrollLine.mj_w = oldTitleView.mj_w + wDistance * progress;
    ...
    if (self.coverLayer) {
        if (self.segmentStyle.isScrollTitle) {
            self.coverLayer.mj_x = oldTitleView.mj_x + xDistance * progress - xGap;
            self.coverLayer.mj_w = oldTitleView.mj_w + wDistance * progress + wGap;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                CGFloat oldCoverX = oldTitleView.mj_x + (oldTitleView.mj_w - oldCoverW) * 0.5;
                CGFloat currentCoverX = currentTitleView.mj_x + (currentTitleView.mj_w - currentCoverW) * 0.5;
                xDistance = currentCoverX - oldCoverX;
                self.coverLayer.mj_x = oldCoverX + xDistance * progress;
                self.coverLayer.mj_w = oldCoverW + wDistance * progress;
            } else {
                self.coverLayer.mj_x = oldTitleView.mj_x + xDistance * progress;
                self.coverLayer.mj_w = oldTitleView.mj_w + wDistance * progress;
            }
        }
    }
...
- (void)reformScrollLineViewPositionCurrentIndex:(NSInteger)currentIndex{
    if (self.scrollLine.centerX > kScreenWidth/self.titleWidths.count/2 +6 &&
        currentIndex == 0){
        [UIView animateWithDuration:0.05 animations:^{
            self.scrollLine.centerX = kScreenWidth/self.titleWidths.count/2;
        }];
    }
}
...
- (void)adjustTitleOffSetToCurrentIndex:(NSInteger)currentIndex {
    ...
    int index = 0;
    for (BKPageTitleView *titleView in _titleViews) {
...
//
    ...
    if (self.scrollView.contentSize.width != self.scrollView.bounds.size.width + contentSizeXOff) {// 需要滚动
        BKPageTitleView *currentTitleView = (BKPageTitleView *)_titleViews[currentIndex];
        ...
        CGFloat extraBtnW = self.extraBtn ? self.extraBtn.mj_w : 0.0;
