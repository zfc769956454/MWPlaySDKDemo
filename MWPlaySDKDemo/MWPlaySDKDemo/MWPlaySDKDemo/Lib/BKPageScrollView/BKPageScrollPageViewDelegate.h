//
//  BKPageScrollPageViewDelegate.h
//  ZJScrollPageView
//
...
@class BKPageContentView;
@class BKPageTitleView;
@class BKPageCollectionView;
 ...
 * 或者'BKPageScrollPageViewDelegate'提供的代理方法
 */
...
@protocol BKPageScrollPageViewDelegate <NSObject>
/** 将要显示的子页面的总数 */
- (NSInteger)numberOfChildViewControllers;

/** 获取到将要显示的页面的控制器
 * -reuseViewController : 这个是返回给您的controller, 您应该首先判断这个是否为nil, 如果为nil 创建对应的控制器并返回, 如果不为nil直接使用并返回
 * -index : 对应的下标
 */
...
- (void)setUpTitleView:(BKPageTitleView *)titleView forIndex:(NSInteger)index;
