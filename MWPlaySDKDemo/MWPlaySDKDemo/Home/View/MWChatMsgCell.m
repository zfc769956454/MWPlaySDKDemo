...
- (void)refreshCellWithInfo:(MWCommentData *)info{
    ...
    if([info.sockData.isAdmin integerValue] == 1 &&
       !ObjIsEqualNSNull(info.sockData.liveinfo.admin_tag)){//设置了主播的身份
        if(info.sockData.liveinfo.admin_tag.length != 0) {//先判断NSNull才能有此判断
            self.administratorRemarkLabel.text = info.sockData.liveinfo.admin_tag;
            NSDictionary *dic = @{NSFontAttributeName:self.administratorRemarkLabel.font};
            CGSize adSize = [self.administratorRemarkLabel.text  boundingRectWithSize:CGSizeMake(MAXFLOAT, 16) options:NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size;
            [self.administratorRemarkImageView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.height.mas_equalTo(16);
                make.width.mas_equalTo(adSize.width + 7);
            }];
        }else {
            [self.administratorRemarkImageView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.height.mas_equalTo(0);
